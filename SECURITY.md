...
- Workspace setup reads only the invoking human command's `agent.session.header.cwd`, resolves its real Git root, and never falls back to the host process cwd. Publisher commands, including explicit profile names, must match that workspace. No source, config path or relay override is accepted in chat.
- Device bootstrap and peer trust changes are trusted-terminal operations. Workspace setup may reference already registered identities and peers but cannot generate keys or change their trust. Profiles persist resolved trust: changing a default peer does not silently rewrite existing profiles.
- Init previews bind the exact cwd, Git root and index, include/exclude, identity/peers, complete candidate config, and a revision covering current config bytes and configured key contents. Confirmation expires after 15 minutes. Config writes use a per-config process lock, full candidate validation, a fresh revision check, fsync and atomic rename. Concurrent/expired previews fail closed; no in-place JSON edits occur.
- New profiles reject duplicate same-role project bindings, overlapping sources/targets and existing state namespaces. Receiver clones copy configuration only. Removing a profile retains its state and reserves the old name/binding so a new profile cannot quietly reset replay protection.
- Git inspection disables fsmonitor as well as hooks. Setup suggestions omit secret-like/nonportable paths and symlink/submodule entries; humans still review includes and omitted files. Suggestion heuristics are not a secret scanner.
...
- Configuration preview records live in the private `config.json.control` directory. The config lock is distinct from project locks. If a process dies while holding it, use trusted-terminal `config unlock` after checking that the owner process is gone. A rename can already have committed if a later directory sync fails: inspect the resulting config before retrying.
- Version 0.2 reads original v1 configs without rewriting projects or state. Adding optional `device` metadata is not readable by the old 0.1 binary; do not downgrade by restoring stale replay state. CLI `init` now aliases device bootstrap; only `/sp init` creates workspace profiles.
