...
export async function git(p, args, options = {}) {
  return run(p.binaries.git, ['--no-pager','--no-optional-locks','-c','core.hooksPath=/dev/null','-c','core.fsmonitor=false','-c','commit.gpgsign=false','-c','tag.gpgsign=false','-c','protocol.file.allow=never',...args], options);
}
...
export async function snapshot(p, signal) {
  const cwd = p.source;
  const top=(await git(p,['rev-parse','--show-toplevel'],{cwd,signal,limit:8192})).stdout.toString().replace(/\r?\n$/,'');
